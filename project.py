...
# def profile_data(df):
#     profile = ydata_profiling.ProfileReport(df, title="Data info")
#     st_profile_report(profile)

# def check_missing_values(df, column):
#     null_count = df[column].isnull().sum()
#     if null_count == 0:
#         st.warning(f"No Null Values in {column}")
#     return null_count
...
# def plot_graph(df, x, y, plot_type, color=None):
#     plot = None
#     if plot_type == "Line":
#         plot = px.line(df, x=x, y=y, color=color, markers=True)
#     elif plot_type == "Scatter":
#         plot = px.scatter(df, x=x, y=y, color=color)
...
# with st.sidebar:
#     st.header("Import Data")
#     file_method = st.radio("Select method to fetch data:", ["Browse Files", "Link/Name"])
#     if file_method == "Browse Files":
#         data_file = st.file_uploader("Upload File:")
#         df = load_data(data_file)
#     elif file_method == "Link/Name":
#         dataset_name = st.text_input("Enter dataset name from seaborn:")
#         df = sns.load_dataset(dataset_name) if dataset_name else None
#     if df is not None:
#         st.dataframe(df)
...
#         st.header("Data Cleaning")
#         if stateful_button("Missing Values", use_container_width=True, key="nill"):
#             method = st.selectbox("Select method to handle Null values:", ["Drop", "Mean", "Median", "Mode"])
...
#         plot_type = st.radio("Select plot type:", ["Line", "Scatter", "Bar", "Pie"])
#         plot_graph(df, x_axis, y_axis, plot_type)

# # Model Building
# with tab4:
#     if df is not None:
#         st.header("Model Building")
#         model_type = st.radio("Select model type:", ["Random Forest", "Naive Bayes", "Decision Tree", "KNN", "SVM", "Linear Regression", "Logistic Regression"])
#         model_dict = {
...
#         X = df[st.multiselect("Select Independent Values (X):", df.columns)]
#         y = df[st.multiselect("Select Dependent Values (Y):", df.columns)]
#         X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=30)
...
#         test_values = [float(st.text_input(f"Enter value for {col}:", key=col)) for col in X.columns]
